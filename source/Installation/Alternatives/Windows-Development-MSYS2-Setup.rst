.. redirect-from::
   ...
   Installation/Windows-Development-MSYS2-Setup

Windows (MSYS2, source)
================
...
This guide is about how to setup a development environment for ROS 2 on Windows with MSYS2 (https://www.msys2.org/).
...
.. include:: ../_Windows-Install-Prerequisites-MSYS2.rst
...
First setup a development folder, for example ``C:\ros``:
...
.. code-block:: bash
   ...
   cd /c/ros/
   git clone https://github.com/ros2/ros2.git -b {DISTRO} {DISTRO}

 .. note::
   `cd /c/ros/` is the equivalent of `cd C:\ros\` in Windows command prompt.

.. note::
   Now the ``ros2.repos`` file should be found at ``C:\ros\{DISTRO}\ros2.repos`` and an empty ``src`` folder should be created at ``C:\ros\{DISTRO}\src``.
...
.. code-block:: bash
   ...
   cd /c/ros/{DISTRO}
   colcon build --merge-install --symlink-install  --cmake-args -G "Ninja" --event-handlers console_direct+

This will build all the packages in the workspace, and install them to the ``c:\ros\{DISTRO}\install`` folder.
...
.. note::
   ...
   ``-G "Ninja"`` is used to if you don't have Visual Studio installed.
   If you have Visual Studio installed, you can omit this argument and let colcon choose the appropriate generator.
   In that case, you need to open a Visual Studio command prompt and run the command from there.
